...
{
    public static IServiceCollection ConfigureFeatureToggle(this IServiceCollection services, IConfiguration configuration)
    {
    ...
    private static IServiceCollection UseConfigFeatureFlags(this IServiceCollection services, IConfiguration configuration)
    {
	    ...
	    Log.Debug("ConfigFeatureToggle initializizing with {Count} flags", featureFlags.Count);
    ...
    private static LdClient GetLaunchDarklyClient(IServiceProvider serviceProvider)
    {
        ...
        Log.Debug("LaunchDarkly client created.");
