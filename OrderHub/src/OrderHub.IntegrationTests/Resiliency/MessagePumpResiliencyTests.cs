...
namespace OrderHub.IntegrationTests.Resiliency;

/// <summary>
/// Proves that the shared MessagePump pipeline behaves correctly under adverse conditions:
/// SQS visibility backoff, circuit-breaker activation, and retry-result routing.
/// Tests 2 and 3 are pure in-memory and run in every environment without any
/// infrastructure dependency.  Test 1 requires SQS — it auto-skips when the
/// configured endpoint is not reachable (LocalStack not running or no AWS creds).
/// </summary>
[Collection("ApiTests")]
public class MessagePumpResiliencyTests(ApiTestsFixture fixture)
{
    // ─────────────────────────────────────────────────────────────────────────
    // Test 1 — SQS visibility timeout: RetryMessageAsync hides the message
    // ─────────────────────────────────────────────────────────────────────────
    ...
    public async Task RetryMessageAsync_WithBackoff_HidesMessageUntilVisibilityTimeout()
    {
        ...
        try
        {
            ...
            Assert.Empty(immediateReceive.Messages);
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // Test 2 — Circuit breaker: opens after threshold and inserts break delay
    // ─────────────────────────────────────────────────────────────────────────
    ...
    public async Task QueueMessageWorker_OpensCircuit_AppliesBreakDelay_ThenSkipsHandler()
    {
        ...
        Assert.True(stopwatch.Elapsed >= TimeSpan.FromMilliseconds(900),
            $"Expected circuit break delay ≥ 900ms, actual: {stopwatch.Elapsed}.");
        Assert.Equal(1, handler.CallCount);
    }

    // ─────────────────────────────────────────────────────────────────────────
    // Test 3 — Retry routing: MessageResult.Retry routes to RetryMessageAsync
    // ─────────────────────────────────────────────────────────────────────────
    ...
    private async Task<IAmazonSQS> CreateSqsClientAsync()
    {
        ...
        {
            Assert.True(await IsServiceAvailableAsync(serviceUrl),
                $"Configured SQS endpoint is unreachable: {serviceUrl}");
    ...
    private sealed class NoOpQueueClient : IQueueClient<string>
    {
        ...
        public Task<List<string>> GetMessagesAsync(int maxNumberOfMessages, CancellationToken cancellationToken = default) =>
