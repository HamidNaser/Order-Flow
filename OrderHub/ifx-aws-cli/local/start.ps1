...
function Show-ServiceSummary {
    ...
    $services = @(
        ...
        @{ Name = "MongoDB"; URL = "mongodb://localhost:$mongodbPort"; Description = "Database" }
        @{ Name = "Redis"; URL = "redis://localhost:$redisPort"; Description = "Cache" }
        @{ Name = "Keycloak"; URL = "http://localhost:$keycloakPort"; Description = "OAuth2/OpenID Provider" }
    )
...
try {
    ...
    # Always run initialization containers since LocalStack persistence is a paid feature
    # and AWS resources need to be recreated on each start
    Write-Info "Starting Docker Compose services with initialization..."
    docker-compose up -d
    ...
    if (-not (Wait-ForService -ServiceName "mongodb" -TimeoutSeconds 60)) {
        Write-Error "MongoDB failed to start. Check logs with: docker-compose logs mongodb"
        exit 1
    }
    ...
    if (-not (Wait-ForService -ServiceName "keycloak" -TimeoutSeconds 90)) {
        Write-Error "Keycloak failed to start. Check logs with: docker-compose logs keycloak"
        exit 1
    }
    ...
    # Wait for mongodb-migrations to complete
    $migrationsStatus = docker-compose ps -a --format json mongodb-migrations 2>$null | ConvertFrom-Json
    if ($migrationsStatus) {
        if (-not (Wait-ForContainer -ContainerName "mongodb-migrations" -TimeoutSeconds 120)) {
            Write-Error "MongoDB migrations failed"
            Write-Info "Check logs with: docker-compose logs mongodb-migrations"
            exit 1
        }
    }
    else {
        Write-Warning "mongodb-migrations container not found (may not be configured yet)"
    }
    ...
    $requiredServices = @("localstack", "mongodb", "redis", "keycloak")
