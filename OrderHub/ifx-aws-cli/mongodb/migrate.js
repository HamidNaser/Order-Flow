...
var hello = adminDb.runCommand({ hello: 1 });
var isMongos = hello && hello.msg === "isdbgrid";

if (!isMongos) {
    print("[ MongoDB Migration | orders ] Skipping sharding (not connected to mongos).");
} else if (!db.orders.stats().sharded) {
    print("[ MongoDB Migration | orders ] Sharding collection on { StoreId: 1 }...");
    sh.shardCollection(dbName + ".orders", { StoreId: 1 });
} else {
    print("[ MongoDB Migration | orders ] Collection already sharded.");
}

print("[ MongoDB Migration ] Migrations completed successfully!");
