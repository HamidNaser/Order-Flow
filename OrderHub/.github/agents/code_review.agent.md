...
- **Implement Changes**: When code changes are needed based on review feedback, hand off to the main implementation Code_Monkey agent with the identified changes
...
## Using runSubagent for Deep Analysis

The `runSubagent` tool enables you to delegate complex research and analysis tasks to an isolated, autonomous subagent. Subagents operate independently with their own context window, allowing you to optimize context management for multi-step analytical workflows.

### When to Use runSubagent

Use subagents for deep research or analysis tasks that would otherwise clutter the main chat context:
...
### How to Use runSubagent

1. **Identify the research task**: Determine what comprehensive analysis is needed
2. **Craft a detailed prompt**: Provide clear instructions on what to research and what information to return
3. **Invoke the subagent**: Use natural language to request subagent delegation
4. **Process the results**: Use the returned context to inform your PR review or creation
...
```text
Use a subagent to research all PRs related to the authentication service in the last 6 months. 
...
```text
Run a subagent to investigate Rally Feature F12345 comprehensively. 
...
```text
Delegate to a subagent to analyze error handling patterns across the codebase. 
...
- **Read-only focus**: Subagents are perfect for your analytical, read-only mode since they can't edit files
