...
.PARAMETER Service
    Optional service name to show logs for (localstack, redis, terraform-init).
...
.EXAMPLE
    .\logs.ps1 -Service redis -Follow
    Stream Redis logs in real-time
...
param(
    ...
    [ValidateSet('localstack', 'redis', 'terraform-init')]
    [string]$Service,
